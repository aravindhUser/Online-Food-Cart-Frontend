...
@Component({
  selector: 'app-registration-page-user',
  imports: [CommonModule, ReactiveFormsModule, RouterModule],
  templateUrl: './registration-page-user.html',
  styleUrl: './registration-page-user.css',
})
export class RegistrationPageUser implements OnInit , OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.registrationForm.get('userPassword')?.valueChanges.subscribe(password => {
      this.checkPasswordStrength(password);
    });
    
    // Debounced email checking
    this.emailCheckSub = this.registrationForm.get('userEmail')?.valueChanges
      .pipe(
        debounceTime(500),
        distinctUntilChanged()
      )
      .subscribe(email => {
        if (this.f['userEmail'].valid) {
          this.checkEmail();
        }
      });
  }
